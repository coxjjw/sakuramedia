...
<a id="jackett-public-indexers"></a>

### 没有 PT 站时，Jackett 先接哪些公开索引器？

结论：作为最小兜底组合，可以先在 Jackett 里加上 `Knaben` 和 `sukebei.nyaa.si` 这两个公开索引器，能覆盖大部分番号，等有条件再切到 PT 站作为主力。

说明：

- 两个索引器都自带在 Jackett 的公开索引器列表里，直接搜索名称即可添加，不需要注册、不需要登录
- `Knaben` 是聚合型索引器，覆盖面较广，绝大部分番号都能搜到候选
- `sukebei.nyaa.si` 中文字幕资源比例较高，命中率也不错
- 两个站点都没有地域限制，Jackett 侧直连即可，不需要额外代理
- 这只是兜底方案：认真使用自动化收藏还是强烈建议接入 PT 站（如 M-Team）作为主力索引器，公开索引器整体做种健康度和速度都不稳定，详见 [收藏思路 → 强烈建议接 PT 站](/guide/collection-strategy#强烈建议接-pt-站-而不是只用-bt)
