...
| LLM/翻译 | `/config`(读写 `movie_info_translation` 节) + `POST /movie-desc-translation-settings/test`(连通性探测) | 读走 `GET /config` 取 `values.movie_info_translation`;写走 `PATCH /config` 传 `{ "movie_info_translation": {...} }`;测试走独立 `/test` 端点 |
| 高级设置 | `/config`(读写 media / metadata / scheduler / downloads / logging 五节) | 父 section 单次 `GET /config` 后分发到 4 张卡；每张卡独立 partial `PATCH /config`；敏感的非目标节不进入前端 DTO |
...
1. **`_categories` 顺序 = IndexedStack 索引 = `active: _selectedIndex == N` 的硬编码 N**(三处)。插入/重排分类必须同步改三处,否则 active 判断错位、错误 tab 发请求。当前「高级设置」是 index 6，「媒体维护」是 index 7。`itemKey` 沿用原 tab key 保持深链/测试兼容。
...
5. **LLM 桌面/移动逻辑各一份**(独立 `_LlmConfigTestState` enum,及**逐字重复**的表单逻辑:校验函数、DTO 组装、加载/保存/连通性测试流程),改校验/流程两边都改。注:URL 校验已下沉共享 `isValidHttpUrl`(`core/validation/url_validators.dart`),不再是两份私有实现。
...
- **番号特征规范化下沉到后端 `Media.others_number_features` 模型层**(2026-07 起),前端在「高级设置」中只提供 mock 预览,权威结果以 `/config` 保存后返回为准；影片详情只按 `MovieCollectionStatusDto.isCollection` 标记"标记为合集/单体"。
